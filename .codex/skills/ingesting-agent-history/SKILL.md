...
description: Creates agent ingest summaries in .codex/adr/agent_ingest after chat history clears/compacts. Use when closing or resetting a session to preserve context, app state, and next steps.
---

# Ingesting Agent History

Creates a timestamped session summary file in `.codex/adr/agent_ingest/` whenever a chat is cleared or compacted.
...
1. **Create a new ingest file** in `.codex/adr/agent_ingest/` using the template:
